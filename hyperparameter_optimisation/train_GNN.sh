...
MODEL=$2 # DNN, GCN or Graph
DISTANCE=$3 # euclidean, cosine, or emd
EDGE_FRAC=0.1
EPOCH=5
SINGLEFOLD=1
VALFRAC=4
...
model_save_path=$SAVE_DIR"/"$MODEL"_"$VAR_LEVEL"_"$DISTANCE"_EdgeFrac"$EDGE_FRAC"/models/"
                ...
                for dnn_nodes in "${MLP_HIDDEN_NODES[@]}"; do
                    for dnn_layers in "${MLP_LAYERS[@]}"; do
                        ...
                        for gnn_nodes in "${GNN_HIDDEN_NODES[@]}"; do
                            ...
                            gnn_hidden_layers="["
                                for ((i=1; i<=gnn_layers; i++)); do
                                    gnn_hidden_layers+="$gnn_nodes"
                                    if [ $i -lt $gnn_layers ]; then
                                    gnn_hidden_layers+=", "
                                    fi
                                done
                                for ((i=1; i<=dnn_layers+gnn_layers; i++)); do
                                    dropout_rates+="$dropout"
                                    if [ $i -lt $gnn_layers+$dnn_layers ]; then
                                    dropout_rates+=", "
                                    fi
                                done
                                dropout_rates+="]"
                                ...
                                echo "Training with GNN layers: " $gnn_hidden_layers          
